...
function set_up_and_send_new_contact($clean_form_data) {
    ...
    //Check HS to see if contact exists
    ...
    $does_contact_exist = trigger_hubspot_curl($url, $fields, $fields_string);

    //If we have contact, do NOT update in HS; just run checks on payment/coupon then add to list
    if($does_contact_exist->total == 1) {
        $hs_contact_id = $does_contact_exist->results[0]->id;
        file_put_contents('hubspot_log.txt', "FOUND CONTACT ID: " . $hs_contact_id . " IN HS ALREADY \n", FILE_APPEND);
    } else {
    // Add new contact to HS then do checks on payment/coupon, THEN move to list
        ...
        if(isset($response->vid)) {
            $hs_contact_id = $response->vid;
            file_put_contents('hubspot_log.txt', "New contact " . $hs_contact_id . " added to HS." . "\n", FILE_APPEND);
        } else {
    ...
    if(isset($hs_contact_id) && $hs_contact_id !== null && !empty($hs_contact_id)) {   
        add_hubspot_id_to_database($hs_contact_id, $clean_form_data['email']);
        ...
        if($clean_form_data['payment_status'] == "Pending") {
            ...
            set_up_and_send_list_add($hs_contact_id);
            return array("Success", "Contact added to Hubspot event list.");
        }
    } else {
        return array("Dammit", "We could not communicate with Hubspot. Please try again later.");
    }
...
function add_hubspot_id_to_database($hs_id, $email) {
    global $wpdb;
    $my_table = $wpdb->prefix . 'registrations';
     
    $update_hs = $wpdb->query( $wpdb->prepare( 
     "
         UPDATE $my_table 
         SET hubspot_id = %s
         WHERE email = %s
